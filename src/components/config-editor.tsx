"use client";

import { useCallback, useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";

export function ConfigEditor() {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [yaml, setYaml] = useState("");
  const [status, setStatus] = useState<"idle" | "loading" | "saving">("idle");
  const [error, setError] = useState<string | null>(null);
  const textareaRef = useRef<HTMLTextAreaElement>(null);

  const close = useCallback(() => setOpen(false), []);

  async function load() {
    setOpen(true);
    setError(null);
    setStatus("loading");
    try {
      const res = await fetch("/api/config");
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Failed to load config.");
      setYaml(data.yaml ?? "");
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to load config.");
    } finally {
      setStatus("idle");
    }
  }

  async function save() {
    setError(null);
    setStatus("saving");
    try {
      const res = await fetch("/api/config", {
        method: "PUT",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ yaml }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Failed to save.");
      setOpen(false);
      router.refresh();
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to save.");
    } finally {
      setStatus("idle");
    }
  }

  // Escape closes; focus the textarea when the modal opens.
  useEffect(() => {
    if (!open) return;
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") close();
    };
    window.addEventListener("keydown", onKey);
    textareaRef.current?.focus();
    return () => window.removeEventListener("keydown", onKey);
  }, [open, close]);

  return (
    <>
      <button
        type="button"
        className="icon-control"
        onClick={load}
        aria-label="Edit dashboard config"
        title="Edit config"
      >
        <svg
          className="icon-control-icon"
          viewBox="0 0 512 512"
          fill="currentColor"
          aria-hidden="true"
        >
          <path d="M454.6 45.3l12.1 12.1c12.5 12.5 12.5 32.8 0 45.3L440 129.4 382.6 72l26.7-26.7c12.5-12.5 32.8-12.5 45.3 0zM189 265.6l171-171L417.4 152l-171 171c-4.2 4.2-9.6 7.2-15.4 8.6l-65.6 15.1L180.5 281c1.3-5.8 4.3-11.2 8.6-15.4zm197.7-243L166.4 243c-8.5 8.5-14.4 19.2-17.1 30.9l-20.9 90.6c-1.2 5.4 .4 11 4.3 14.9s9.5 5.5 14.9 4.3l90.6-20.9c11.7-2.7 22.4-8.6 30.9-17.1L489.4 125.3c25-25 25-65.5 0-90.5L477.3 22.6c-25-25-65.5-25-90.5 0zM80 64C35.8 64 0 99.8 0 144V432c0 44.2 35.8 80 80 80H368c44.2 0 80-35.8 80-80V304c0-8.8-7.2-16-16-16s-16 7.2-16 16V432c0 26.5-21.5 48-48 48H80c-26.5 0-48-21.5-48-48V144c0-26.5 21.5-48 48-48H208c8.8 0 16-7.2 16-16s-7.2-16-16-16H80z" />
        </svg>
      </button>

      {open && (
        <div
          className="config-modal-overlay"
          role="dialog"
          aria-modal="true"
          aria-label="Edit dashboard config"
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) close();
          }}
        >
          <div className="config-modal">
            <div className="config-modal-header">
              <span className="config-modal-title">dashboard.yaml</span>
              <button
                type="button"
                className="config-modal-x"
                onClick={close}
                aria-label="Close"
              >
                ✕
              </button>
            </div>

            <textarea
              ref={textareaRef}
              className="config-modal-textarea"
              spellCheck={false}
              value={yaml}
              onChange={(e) => setYaml(e.target.value)}
              disabled={status === "loading"}
              placeholder={status === "loading" ? "Loading…" : ""}
            />

            {error && <p className="config-modal-error">{error}</p>}

            <div className="config-modal-actions">
              <button
                type="button"
                className="config-modal-btn"
                onClick={close}
                disabled={status === "saving"}
              >
                Cancel
              </button>
              <button
                type="button"
                className="config-modal-btn config-modal-btn-primary"
                onClick={save}
                disabled={status !== "idle"}
              >
                {status === "saving" ? "Saving…" : "Save"}
              </button>
            </div>
          </div>
        </div>
      )}
    </>
  );
}
